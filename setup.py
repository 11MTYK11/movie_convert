...
def decompress(filename,outpath):
    ...
    try:
        cmd = [resource_path('./binary/7zr.exe'),"x","-mmt=on",f"-o{outpath}","-bsp1","-y",filename]
...
runname = "動画変換君.exe"
installpath = "\\動画変換君"
dlurl = "https://github.com/MT-YK/movie_convert/releases/latest/download/movie_convert.7z"
lnkname = "動画変換君.lnk"
menuname = "動画変換君"
lnkcomment = ""
uninstallname = "動画変換君"
        ...
        if str(userselect) == "1":
            if os.path.exists(DST):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    if gethash(f"{DST}\sample.bin") == gethash(downloadpath):
                        print("変更はありません")
                    else:
                ...
                if yes_no_input("アンインストールしますか?"):
                    ...
                    if os.path.exists(DST):
                        ...
                        if os.name == 'nt':
                            ...
                            home = os.getenv('HOME')
                        if menuset:
                            path = 'Software\\Classes\\*\\shell'
                            try:
                                winreg.DeleteKeyEx(winreg.HKEY_CURRENT_USER,path + f"\\{rightname}\\command")
                                winreg.DeleteKeyEx(winreg.HKEY_CURRENT_USER,path + f"\\{rightname}")
                            except:
                                import traceback
                                traceback.print_exc()
                        desktop_dir = os.path.join(home, 'Desktop')
